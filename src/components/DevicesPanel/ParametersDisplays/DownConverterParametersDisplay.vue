<template>
  <UniversalDisplayComponent :view-fields-list="viewFields"
                             :title="$tc('DevicesParameters.Converter.titleUI', 0, {type: '#' + deviceId})"
    ...
    <div class="lamp-stack__wrapper">
      <div class="lamp-stack">
        <AlarmLamp class="lamp" v-for="alarm in AlarmList" :alarm-status="alarm.valueParameter"
                   :key="alarm.id"
                   :tooltip="$t('DevicesParameters.Converter.' +  Object.keys(AlarmList).find(key => AlarmList[key] === alarm))"
        ></AlarmLamp>
      </div>
    </div>
...
export default {
  name: 'DownConverterParametersDisplay',
  components: {UniversalDisplayComponent, AlarmLamp},
  data () {
    return {
      AlarmList: {},
